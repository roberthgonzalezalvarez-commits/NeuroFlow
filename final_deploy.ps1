...
# 3. Safe strings for titles (using char codes to avoid encoding issues in the script itself)
$a_acute = [char]225
$em_dash = [char]8212
$titlePexels = "Clip Downloader $em_dash Pexels & Pixabay"
$titleFreepik = "Generador de Im${a_acute}genes"

# 4. Fix the TOOLS object
$toolsRegex = '(?ms)const TOOLS = \{.*?\};'
$newTools = @"
const TOOLS = {
  pexels: { title: '$titlePexels', base64: TOOLS_DATA.pexels },
  workflow: { title: 'Flujo de Trabajo (En desarrollo)', base64: TOOLS_DATA.workflow },
  freepik: { title: '$titleFreepik', base64: TOOLS_DATA.freepik },
  videosync: { title: 'VideoSync Editor', base64: TOOLS_DATA.videosync }
};
"@
$content = $content -replace $toolsRegex, $newTools

# 5. Update the actual base64 in TOOLS_DATA.videosync
$videosyncDataRegex = "videosync:\s*'[^']*'"
$content = $content -replace $videosyncDataRegex, "videosync: '$base64'"

# 6. Fix the corrupted renderProjectList function
$newFunc = @'
...
# 7. Save with UTF-8 No BOM
[IO.File]::WriteAllText($filePath, $content, $encoding)

Write-Host "Deployment SUCCESS. Encoding verified."
